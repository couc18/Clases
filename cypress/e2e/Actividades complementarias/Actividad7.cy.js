...
let datosTareas
    ...
    before("traer datos fixture", () => {
        cy.fixture('Actividad7').then(data => {
            datosTareas = data
        })
    })
    beforeEach("Actividad complementaria 7", () => {
     ...
     it("Deberia sacar una foto completa de la pagina del login", () =>{
        cy.get('#task').type(`${datosTareas.tareas.tarea1} {enter}`);
        cy.get('#task').type(`${datosTareas.tareas.tarea2} {enter}`);
        cy.get('#task').type(`${datosTareas.tareas.tarea3} {enter}`);
   ...
   it("Deberia sacar una foto del boton todoList ", () =>{
    ...
    cy.get('#task').type(`${datosTareas.tareas.tarea2} {enter}`);
    cy.get('#task').type(`${datosTareas.tareas.tarea3} {enter}`);
    cy.get('ul[role="list"]').screenshot("")
